...
//
template<int N, int L>
void RS( const uint64_t q[L], 
	     const uint64_t p[L], const uint64_t a_orig[L*2][N], 
	                                uint64_t a_    [L  ][N] ){
	uint64_t a[L*2][N];
	for(int i=0;i<2*L;i++)
	...
	for(int k=L-1;k>=0;k--){
		...
		uint64_t pinv[L];
		for(int j=0;j<L;j++){
			qinv[j]=      inv_mod(p[k]%q[j],q[j])  ;
			pinv[j]=(j<k)?inv_mod(p[k]%p[j],p[j]):0;
		}
		for(int i=0;i<N;i++){
			uint64_t r = a[L+k][i]>(p[k]/2)?1:0;
			for(int j=0;j<L;j++){
				a[j][i]=mul_mod((a[j][i]+q[j]-(a[L+k][i]%q[j]))%q[j],qinv[j],q[j]);
				if(k==0)
					a_[j][i]=(a[j][i]+r)%q[j];
			}
			for(int j=0;j<k;j++)
				a[L+j][i]=mul_mod((a[L+j][i]+p[j]-(a[L+k][i]%p[j]))%p[j],pinv[j],p[j]);
		}
	}
}
