...
void main(){
	...
	// SlotToCoeff
	//---------------------------------------------------------------------
	uint64_t ct_hat_stc[2][L-13][N];
	
	SlotToCoeff_logN_10<L-10,DNUM,K>(q,p,Delta,A,rkey_hat,ct_hat_evalmod,ct_hat_stc);

	//-----------------------------------------------
	// debug
	//-----------------------------------------------
	{
		uint64_t pt_stc[L-13][N];
		double wr[N/2], wi[N/2];
		dec<N, L-13>(ct_hat_stc, s, q, pt_stc);
		decode<N, logN, L-13>(pt_stc, Delta, q, wr, wi);
